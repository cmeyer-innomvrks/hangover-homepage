...
class EventDetailCard extends View {
  ...
  displayEvent() {
    ...
    card.querySelector(".profile-views").textContent = this.event.watched;
    card.querySelector(".profile-likes").textContent = this.event.saved;
    for (let i = 0; i < locations.length; i++) {
      if (locations[i].id === this.event.locationid) {
        this.location = locations[i];
        card.querySelector(".location-name").textContent =
          "@" + locations[i].name;
        break;
      }
    }
