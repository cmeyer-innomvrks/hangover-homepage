...
class LocationDetailEvents extends View {
    ...
    displayEvents() {
        if (this.events.length > 0) {
            ...
            this.element.innerHTML = "Diese Location hat leider im Moment keine Veranstaltungen.";
        }
    }
    ...
    getEventElement(event) {
        ...
        item.querySelector(".event-location").textContent = "@" + this.location.name;
        item.querySelector(".location-art").textContent = this.location.art;

        item.querySelector(".event-time").textContent = event.date + " - " +
            event.time + "Uhr";
