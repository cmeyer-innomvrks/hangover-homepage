...
class LocationDetailCard extends View {
  ...
  setElement(element) {
    ...
    if (JSON.parse(localStorage.getItem("isSignedIn"))) {
      let checkIfSavedEvent = new Event("isLocationSaved", {
        id: this.location.id
      });
      this.notifyAll(checkIfSavedEvent);
    } else {
      this.element.querySelector(".save-event").classList.add("inactive");
      this.element.querySelector(".location-save-text").textContent =
        "Du musst angemeldet sein, um diese Location speichern zu können.";
    }
  }
  ...
  displayLocation() {
    ...
    if (this.location.monday_from !== "") {
      card.querySelector(
        "#monday-from"
      ).textContent = this.location.monday_from;
      card.querySelector(
        ".monday-till"
      ).textContent = this.location.monday_till;
    } else {
    ...
    if (this.location.tuesday_from !== "") {
      card.querySelector(
        "#tuesday-from"
      ).textContent = this.location.tuesday_from;
      card.querySelector(
        ".tuesday-till"
      ).textContent = this.location.tuesday_till;
    } else {
    ...
    if (this.location.wednesday_from !== "") {
      card.querySelector(
        "#wednesday-from"
      ).textContent = this.location.wednesday_from;
      card.querySelector(
        ".wednesday-till"
      ).textContent = this.location.wednesday_till;
    } else {
    ...
    if (this.location.thursday_from !== "") {
      card.querySelector(
        "#thursday-from"
      ).textContent = this.location.thursday_from;
      card.querySelector(
        ".thursday-till"
      ).textContent = this.location.thursday_till;
    } else {
    ...
    if (this.location.friday_from !== "") {
      card.querySelector(
        "#friday-from"
      ).textContent = this.location.friday_from;
      card.querySelector(
        ".friday-till"
      ).textContent = this.location.friday_till;
    } else {
    ...
    if (this.location.saturday_from !== "") {
      card.querySelector(
        "#saturday-from"
      ).textContent = this.location.saturday_from;
      card.querySelector(
        ".saturday-till"
      ).textContent = this.location.saturday_till;
    } else {
    ...
    if (this.location.sunday_from !== "") {
      card.querySelector(
        "#sunday-from"
      ).textContent = this.location.sunday_from;
      card.querySelector(
        ".sunday-till"
      ).textContent = this.location.sunday_till;
    } else {
  ...
  setSavedStatus(status) {
    ...
    this.element
      .querySelector("#save-location")
      .addEventListener("click", this.onSave.bind(this));
  }

  onSave() {
    if (this.saveStatus) {
      let requesUnsaveEvent = new Event("unsaveLocation", {
        id: this.location.id
      });
      this.notifyAll(requesUnsaveEvent);
      this.saveStatus = false;
    } else {
      let requestSaveEvent = new Event("saveLocation", {
        id: this.location.id
      });
