...
function init() {
    ...
    PersonalLocations.setElement(document.querySelector(".profile-location-list"));
    PersonalLocations.addEventListener("unsave", onUnsaveRequestedLoc);
    PersonalLocations.addEventListener("toLocationDetail", onLocationDetailRequested);
...
async function onSignInCompleted() {
    ...
    if (await ProfileLoader.isExisting(JSON.parse(localStorage.getItem("user")))) {
        // TODO
    } else {
