...
class SavedLocationsLoader {
    ...
    async isSaved(id) {
        let flag;
        await database.collection("User").doc(JSON.parse(localStorage.getItem("user")).mail).get().then(function (doc) {
            if (doc.exists) {
    ...
    async save(id) {
        await database.collection("User").doc(JSON.parse(localStorage.getItem("user")).mail).get().then(function (doc) {
            if (doc.exists) {
                ...
                database.collection("User").doc(JSON.parse(localStorage.getItem("user")).mail).set({
